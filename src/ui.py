...
def build_risk_map(df, config):
    ...
    if geojson_path and Path(geojson_path).exists():
        ...
        folium.GeoJson(geojson, style_function=style_function, tooltip=tooltip).add_to(
            m
        )
    else:
...
def render_dashboard(config, df, metrics=None):
    st.markdown(
        """<div class="section-heading">Regional Summary</div>""", unsafe_allow_html=True
    )
    ...
    st.markdown(
        """<div class="section-heading">Risk Distribution</div>""", unsafe_allow_html=True
    )
    ...
    fig = px.bar(
        ...
        y="Count",
        color="Count",
        color_continuous_scale=["#1d293d", "#615fff"],
    )
    ...
    if metrics:
        ...
        st.markdown(
            f"<div class='interp-box'><strong>Auto Interpretation:</strong> Overall model performance is at <strong>{perf_text}</strong> level (Macro F1: {macro_f1:.2f}). This means the system is {('reliable enough' if macro_f1 > 0.6 else 'not reliable enough')} to accurately identify which areas are most vulnerable to environment-based diseases.</div>",
            unsafe_allow_html=True,
        )
...
def render_heatmap(config, df):
    st.markdown(
        """<div class="section-heading">Spatial Risk Heatmap</div>""",
        unsafe_allow_html=True,
    )
    ...
    if len(df) > 1:
        ...
        if not counts.empty:
            ...
            st.markdown(
                f"<div class='interp-box'><strong>Auto Map Interpretation:</strong> Overall, the region is dominated by <strong>{dominant_str}</strong> category which covers {pct:.1f}% of the area in the current dataset. This distribution pattern can help health services focus budget and healthcare workforce allocation.</div>",
                unsafe_allow_html=True,
            )
...
def render_simulation(config, df, artifacts):
    st.markdown(
        """<div class="section-heading">What-If Analysis</div>""", unsafe_allow_html=True
    )
...
def render_data(config, df):
    ...
    with tab_raw:
        ...
        st.dataframe(
            df.rename(columns=FEATURE_LABELS), use_container_width=True, height=400
        )
    with tab_dist:
        col_ctrl, _ = st.columns([2, 3])
        feature = col_ctrl.selectbox(
            "Feature to inspect:", features, format_func=fl, key="hist_feature"
        )
        ...
        if interp_text:
            st.markdown(
                f"<div class='interp-box'>{interp_text}</div>", unsafe_allow_html=True
            )
    with tab_target:
        col_ctrl2, _ = st.columns([2, 3])
        box_feature = col_ctrl2.selectbox(
            "Numeric feature:", features, format_func=fl, key="box_feature"
        )
        fig_box = px.box(
            df_plot,
            x=status_col,
    ...
    with tab_corr:
        ...
        corr_text = generate_correlation_interpretation(
            df_plot, target_col, features, config
        )
        if corr_text:
            st.markdown(
                f"<div class='interp-box'>{corr_text}</div>", unsafe_allow_html=True
            )

...
raw_file = Path("data/raw/Data Kualitas Air Gabungan.csv")
processed_file = Path("data/processed/water_quality.csv")
processed_file.parent.mkdir(parents=True, exist_ok=True)

# Data structures
# data_by_loc[lokasi] = {'latitude': lat, 'longitude': lon, 'kondisi': kondisi, 'features': {param: value}}
data_by_loc = {}
...
# Count feature occurrences to select robust features
feature_counts = {}
...
# Select robust features (at least 20% coverage)
threshold = len(data_by_loc) * 0.2
valid_cols = [param for param, count in feature_counts.items() if count > threshold]

# Write to CSV
headers = ['lokasi_sampling', 'latitude', 'longitude', 'kondisi', 'risk_class'] + valid_cols
